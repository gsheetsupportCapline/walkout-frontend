...
import "./ArchiveRadioButtonManagement.css";

const ArchiveRadioButtonManagement = () => {
  ...
  const fetchArchives = async () => {
    setLoading(true);
    try {
      const response = await api.get("/radio-buttons/archives/button-sets", {
        params: {
      ...
      showError(
        err.response?.data?.message || "Failed to fetch archived button sets"
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleViewDetails = async (archive) => {
    try {
      const response = await api.get(
        `/radio-buttons/archives/button-sets/${archive._id}`
      );
      if (response.data.success) {
  ...
  const handleRestore = async () => {
    try {
      ...
      const response = await api.post(
        `/radio-buttons/archives/button-sets/${selectedArchive._id}/restore`,
        payload
      );
      if (response.data.success) {
        showSuccess("Button set restored successfully");
      ...
      if (
        ...
        showError(
          err.response?.data?.message || "Failed to restore button set"
        );
      }
    }
  };
  ...
  const handlePermanentDelete = async () => {
    try {
      const response = await api.delete(
        `/radio-buttons/archives/button-sets/${selectedArchive._id}/permanent`
      );
      if (response.data.success) {
        showSuccess("Archived button set permanently deleted");
  ...
  return (
    <div className="archive-management">
      <div className="archive-header">
        <div>
          <h2>Archived Radio Button Sets</h2>
          <p className="archive-subtitle">
            View, restore, or permanently delete archived button sets
          </p>
        </div>
      ...
      {loading ? (
        <div className="loading">Loading archived button sets...</div>
      ) : archives.length === 0 ? (
        <div className="no-archives">
          <p>No archived button sets found</p>
        </div>
      ) : (
        <>
              ...
              <tbody>
                {archives.map((archive) => (
                  <tr key={archive._id}>
                    ...
                    <td>{archive.buttons?.length || 0}</td>
                    <td>
        ...
        >
          ...
          >
            ...
            <div className="archive-details">
              ...
              <div className="detail-row">
                <label>Last Button ID:</label>
                <span>#{selectedArchive.lastButtonId}</span>
              </div>
              <div className="detail-row">
            ...
            <h4>Buttons ({selectedArchive.buttons?.length || 0})</h4>
            <div className="buttons-list">
              <table className="details-table">
                ...
                <tbody>
                  {selectedArchive.buttons?.map((button, index) => (
                    <tr key={index}>
      ...
      {showRestoreModal && selectedArchive && (
        ...
        >
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <h3>Restore Button Set</h3>
            <p className="info-text">
              This will restore the button set and all its buttons back to the
              main collection.
            </p>
        ...
        >
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="archive-info">
              ...
              <br />
              <strong>Buttons:</strong> {selectedArchive.buttons?.length || 0}
              <br />
...
export default ArchiveRadioButtonManagement;
